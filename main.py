...
parser.add_argument('--use_auto_device_map', type=bool, default=True,
...
parser.add_argument('--UniBias', type=bool, default=False, help='Using UniBias or Not')
parser.add_argument('--Calibration', type=bool, default=False, help='Evaluate Calibration Methods or Not')
...
if use_auto_device_map:
    ...
    model = AutoModelForCausalLM.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", **model_kwargs)
else:
    os.environ["CUDA_VISIBLE_DEVICES"] = cuda_device_id
    model = AutoModelForCausalLM.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", **model_kwargs).to(device)

tokenizer = AutoTokenizer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", token=hf_token if hf_token else None)
...
record_file_path = f'./results/{model_id}/{dataset_name}/seed{seed_value}.json'
...
def main():
    ...
    write_json(record_file_path, dataset_name + ' seed_value: ' + str(seed_value) + " num shot: " + str(num_shot) + " repE: " + str(RepE))

    # evaluate performance
    final_acc, all_label_probs, cf = ICL_evaluation(model, prompt_list, test_labels, gt_ans_ids_list, dataset_name, repE=RepE)
