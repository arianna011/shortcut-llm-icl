...
\begin{figure}[ht]
    ...
    \caption{Multi-run confusion matrices of the hyperparameter search evaluation runs on the RTE dataset illustrating the effect of RepControl intervention strength ($\alpha$) on model predictions.}
...
\begin{equation}
\mathrm{IG}_i(x) = (x_i - x'_i) \cdot \int_{\beta=0}^{1} 
\frac{\partial F\!\left(x' + \beta (x - x')\right)}{\partial x_i} \, d\beta
\end{equation}

\noindent In practice, the integral is approximated via a Riemann sum over $m$ discrete interpolation steps:
\begin{equation}
...
\noindent In this work, the baseline $x'$ is chosen as the embedding of the beginning-of-sequence token, which provides a stable and semantically neutral reference compared to an all-zero embedding.
