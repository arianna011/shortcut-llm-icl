...
%DA REVISIONARE
In particular, this study aims to answer research questions such as:
\begin{itemize}
    \item is the concept of "taking a shortcut" explicitely encoded by LLMs in their hidden representations, meaning they know more than they reveal with generations, similarly to how they encode truthfulness of statements in the case of hallucinations \citep{azaria2023internalstatellmknows}? 
    \item can a "shortcut reliance" linear direction be found in the latent representation space with methods such as Principal Component Analysis (PCA)?
    \item can the steering of model's predictions at inference time via targeted direction amplification or suppression lead to more robust answers?
\end{itemize}
The answers to these questions would determine whether it is possible to mitigate shortcut learning by leveraging a lightweight and interpretable method such as RepE, which analyzes and control a model's hidden representations without any parameters update.\\

Several experiments are conducted to test the effectiveness of the RepE framework adapted to the shortcut detection and mitigation task and to analyze the steered model latent representations to assess correct behavior and enhance method transparency. Given the complexity and novelty of this goal, the target case study is a simple one: LLMs under ICL for NLP tasks such as textual entailment.\\
